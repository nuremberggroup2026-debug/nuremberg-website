...
function FormSubmittingButton({ locale, isSubmitting }: Props) {
  ...
  return (
    <div>
      <div className="input-field pt-2">
        ...
        >
          ...
          {/* Borders */}
          <div
            className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-cyan-500/50 
    ...
    transition-all duration-700"
          ...
          <div
            className="absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-cyan-500/50 
    group-hover/btn:w-full group-hover/btn:h-full 
    group-hover/btn:border-cyan-500 
    transition-all duration-700"
          />
          ...
          {/* Content */}
          <div
            className="flex items-center justify-center gap-4 font-black italic uppercase tracking-[0.2em] text-sm 
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span className="animate-pulse text-lg md:text-xl">
                  {isArabic ? "جاري الإرسال..." : "Submitting..."}{" "}
                </span>
              </>
            ) : (
              <>
              <span className="text-lg md:text-xl"> {isArabic ? "تقديم الطلب" : "Submit Application"}
</span>
               
                <div
                  className="p-3 bg-cyan-500 rounded-full 
